...
xref:odraw::index.adoc[OCPN Draw]. OCPN Draw is used to draw points, lines
...
=== Use Plugin Manager (PIM)

Watchdog is available as a managed plugin (PIM), which is the easiest way to install. The OpenCPN Manual has general information in xref:opencpn-plugins:misc:plugin-install.adoc[Plugin Download, Install and Enable] 
...
For any of the alarms, send an email alert.
